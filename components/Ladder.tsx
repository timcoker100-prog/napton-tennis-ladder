...
export default function Ladder() {
  ...
  const openProfile = () => {
    // For now we use a simple prompt - you can expand later
    const name = prompt("Your full name:");
    if (name) {
      // You can add more profile logic later
      alert("Profile updated (basic version)");
    }
  };
    ...
    <div className="min-h-screen bg-emerald-50 p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap gap-3 justify-center mb-8">
          ...
          <button onClick={openProfile} className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl">👤 My Profile</button>
          ...
          {/* Admin Button - only visible if you want, but protected */}
              ...
              {sortedPlayers.map((player, index) => (
                <tr key={player.email} className="border-b hover:bg-emerald-50">
                  ...
                  <td className="px-6 py-4 text-center">
                    <button onClick={() => removePlayer(player.email)} className="text-red-500 hover:text-red-700">Remove</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showAdminModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 max-w-lg w-full">
            ...
            <button onClick={() => setShowAdminModal(false)} className="w-full py-3 text-gray-500">Close</button>
          </div>
        </div>
      )}
    </div>
